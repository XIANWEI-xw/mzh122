...
// ===== 安全拖动+点击系统 =====
function makeOrbDraggable(el, tapFn) {
    if (!el) return;
...
(function() {
    function setup() {
        makeOrbDraggable(document.getElementById('apiFab'), function() { openApiMonitor(); });
        makeOrbDraggable(document.getElementById('timerOrb'), function() { openTimerPanel(); });
    }
...
function openTimerPanel() {
    ...
    setTimeout(() => { _tpCanClose = true; }, 600);
}

function closeTimerPanel() {
    if (!_tpCanClose) return;
    const overlay = document.getElementById('tpOverlay');
...
let _amCanClose = false;

function openApiMonitor() {
    ...
    renderAmStats();
    renderAmLogs();
    setTimeout(() => { _amCanClose = true; }, 600);
}

function closeApiMonitor() {
    if (!_amCanClose) return;
    const overlay = document.getElementById('apiMonitorOverlay');
    if (overlay) overlay.classList.remove('active');
    _amCanClose = false;
}
...
// （已删除，逻辑合并到 closeApiMonitor / closeTimerPanel 的 _canClose 标志位中）
